...
def test_evaluator_reports_benchmark_dimensions() -> None:
    ...
    analysis = client.post("/api/analyze", json={"scenarioName": "officeToPowerShell", "reportMode": "template"})
    assert analysis.status_code == 200

    payload = analysis.json()
    evaluation = client.post(
        "/api/evaluate",
        json={"analysisId": payload["analysisId"], "scenarioName": "officeToPowerShell"},
    )
